...
@magic_factory(saving_folder={'mode': 'd'})
def set_save_results(save_results:bool,saving_folder:pathlib.Path = pathlib.Path.home()):
...
class ContrastWidget(QWidget):
    ...
    def create_ui(self):
        ...
        self.set_save_results_widget = set_save_results(call_button=False)
        ...
        self.add_magic_function(self.set_save_results_widget, layout)
    ...
    def graph_view(self):
        ...
        save_data = self.set_save_results_widget.save_results.value
        if save_data:
            wb = Workbook()
            sheet = wb.active
            ...
            dirname = self.set_save_results_widget.saving_folder.value
            image_name = self.choose_image_widget.Image.value.name
            path = os.path.join(dirname, f'{image_name}.xlsx')
            wb.save(path)
