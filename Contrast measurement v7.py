...
import napari
from qtpy.QtWidgets import QWidget, QPushButton, QDoubleSpinBox, QComboBox, QVBoxLayout, QFormLayout
...
class ContrastWidget(QWidget):
    ...
    def create_ui(self):
        ...
        layout.addRow('Select Shape', shapes_combo)
        ...
        calculate_btn.clicked.connect(lambda: self.calculate_contrast(cam_noise_box.value(), pixel_width_box.value()))
        layout.addWidget(calculate_btn)
        
        # Mettre à jour les options des combobox une fois au démarrage
    ...
    def update_combobox_options(self, event=None):
        ...
        # Ajouter les nouvelles options en fonction des couches de Napari
    ...
    def calculate_contrast(self, camera_noise, pixel_width) -> Labels:
        ...
        # Récupérer les couches sélectionnées à partir des noms
        shape_layer = self.viewer.layers[selected_shape]
        image_layer = self.viewer.layers[selected_image]
        
        global viewer
        Image_data = np.array(image_layer.data[5])
